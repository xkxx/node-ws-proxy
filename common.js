...
exports.enableSSL = function(key, cert) {
	var result = {};
	var key_file = getFileContent(key);
	if (key_file[0]) {
		throw 'Failed to read server key: ' + key_file[0];
	}
	result.key = key_file[1];
	var cert_file = getFileContent(cert);
	if (cert_file[0]) {
		throw 'Failed to read server cert: ' + cert_file[0];
	}
	...
	if (typeof settings ==  'string') {
		...
		if (f[0]) {
			...
			try {
				...
				msg = 'Parsing config failed: ' + e.type;
				settings = {};
			}
		}
	}
	else if(!settings) {
		msg = "Config not provided. Will use default settings";
		settings = {};
	}
